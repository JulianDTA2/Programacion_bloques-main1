...
# --- ROBOTICMINDS portable defaults ---
import os, sys
...
# Rutas por defecto cuando corre “congelado” (.exe). No pisan si ya están definidas.
os.environ.setdefault('ARDUINO_IDE_PATH', resource_path('arduino-1.8.19'))
os.environ.setdefault('ARDUBLOCKLY_ROOT',    resource_path('ardublockly'))
os.environ.setdefault('BLOCKLY_ROOT',        resource_path('blockly'))
os.environ.setdefault('CLOSURE_LIBRARY_ROOT',resource_path('closure-library'))
# --- end ROBOTICMINDS portable defaults ---
import io
...
def main():
    print('Running Python %s (%s bit) on %s' % (
    ...
    # 🔹 base_dir = carpeta _internal (donde están ardublockly, arduino-1.8.19, blockly, etc.)
    base_dir = Path(resource_path())          # ej: D:\roboticlab\RoboticLab\_internal
    ardublockly_root_dir = base_dir / 'ardublockly'

    # Validar index.html
    if not (ardublockly_root_dir / 'index.html').is_file():
        print('❌ ERROR: No se encontró ardublockly/index.html en:')
        print(ardublockly_root_dir)
        sys.exit(1)

    print('Ardublockly root directory:\n\t%s' % ardublockly_root_dir)

    # 🔹 El directorio de trabajo y el document_root deben ser la carpeta base (_internal)
    os.chdir(base_dir)
    print('Current working directory set to:\n\t%s' % os.getcwd())

    server_root = str(base_dir)
